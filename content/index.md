---
title: "Void Sector Archive"
description: "A collection of recovered documents, leaked communications, and suppressed reports relating to substrate mining incidents in the Void Sectors."
tags:
  - index
  - navigation
---

<div class="framed" style="text-align: center; margin-bottom: 2rem;">

![Void Sector](/static/void-sector-logo.svg)

</div>

> *"The universe did not begin with a bang. It began with a wound."*

---

## What Is This?

This archive contains documents relating to a pattern of incidents involving [[concepts/substrate|substrate]] miners operating in [[locations/void-sectors|Void Sectors]]—and the [[concepts/united-mining-guild|corporate cover-up]] that followed.

**19 miners** have vanished in 18 months. The [[concepts/united-mining-guild|United Mining Guild]] knows. They voted not to investigate.

> [!warning] Classification Notice
> Some documents in this archive were obtained through unauthorized disclosure. The UMG considers possession of certain materials a violation of corporate bylaws. Read at your own risk.

---

## Start Here

New to the archive? Read these first:

1. **[[the-fracture|The Fracture]]** — Understand what Void Sectors are and where substrate comes from
2. **[[personal-log-maven-cheung|Maven Cheung's Log]]** — Experience what happens to miners firsthand
3. **[[incident-report-7743|Incident Report 7743]]** — See how the Guild covers it up
4. **[[board-meeting-minutes-2407-01-15|Board Meeting Minutes]]** — Watch them vote against investigation
5. **[[the-leak-encrypted-message|The Leak]]** — Read the whistleblower's warning

---

## Documents

<img src="/static/divider-circuit.svg" alt="" class="divider-circuit" />

### Primary Sources

| Document | Type | Classification |
|----------|------|----------------|
| [[the-fracture\|The Fracture]] | Lore | <span class="badge badge--public">Public</span> |
| [[personal-log-maven-cheung\|Personal Log: Maven Cheung]] | Recovered | <span class="badge badge--public">Public</span> |
| [[incident-report-7743\|Incident Report 7743]] | Official Report | <span class="badge badge--restricted">Restricted</span> |
| [[board-meeting-minutes-2407-01-15\|Board Meeting Minutes]] | Corporate | <span class="badge badge--leaked">Leaked</span> |
| [[the-leak-encrypted-message\|The Leak]] | Whistleblower | <span class="badge badge--classified">Encrypted</span> |

### Reference

- [[timeline|Timeline]] — Chronological event overview

---

## Characters

<img src="/static/divider-substrate.svg" alt="" class="divider-substrate" />

| Character | Role | Status |
|-----------|------|--------|
| [[characters/maven-cheung\|Maven Cheung]] | Independent Miner | <span class="badge badge--missing">Missing</span> |
| [[characters/dr-helena-sarr\|Dr. Helena Sarr]] | Director of Safety | <span class="badge badge--public">Active</span> |

---

## Locations

### Void Sectors

| Sector | Incidents | Risk Level |
|--------|-----------|------------|
| [[locations/void-sectors\|Void Sectors]] (Overview) | 19 total | — |
| [[locations/sector-19-kappa\|Sector 19-Kappa]] | 6 | **EXTREME** |
| [[locations/sector-33-mu\|Sector 33-Mu]] | 4 | Severe |

---

## Concepts

| Concept | Description |
|---------|-------------|
| [[concepts/substrate\|Substrate]] | The material miners extract—and what may be extracting them |
| [[concepts/united-mining-guild\|United Mining Guild]] | The corporation profiting from disappearances |
| [[concepts/deep-space-isolation-syndrome\|Deep Space Isolation Syndrome]] | The convenient explanation |

---

## Browse by Tag

- #leaked — Documents obtained through unauthorized disclosure
- #classified — Restricted access materials
- #maven-cheung — Related to the Cheung incident
- #dr-sarr — Related to the safety review
- #substrate — About the material itself
- #void-sector — About the locations
- #umg — United Mining Guild documents
- #cover-up — Evidence of suppression

---

## The Pattern

> [!danger] What the Guild Won't Tell You
> - **19 incidents** in 18 months
> - **100%** during substrate extraction
> - **100%** solo operations in Void Sectors
> - **100%** psychological deterioration
> - **100%** cargo missing despite extraction logs
> - **73%** black box timestamp anomalies
> - **0** investigations authorized

---

## Contact Protocol

From [[the-leak-encrypted-message|the whistleblower]]:

> *"Post a classified ad in the Meridian-7 Station Bulletin, Section K (Missed Connections), with the phrase:* **'Looking for the friend who left their jacket at the 03:47 shuttle.'**"

---

<div style="text-align: center; margin-top: 3rem;">

![](/static/void-icon.svg){width=80}

</div>

> *"In every Void Sector, you find something else."*
